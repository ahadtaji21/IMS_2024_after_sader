  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        ...
        component: () =>
          import("../views/Gmt/AnnualPlan/GmtAnnualPlanSearchGAP.vue"),
        props: true,
      },
      {
      ...
      },
      {
        ...
        component: () =>
          import("../views/Hrm/Supervisor/HrmTimesheetSupervisor.vue"),
      },
      {
      ...
      },
      {
        ...
        component: () =>
          import("../views/Pms/PmsClassesAttendanceProcessInstance.vue"),
      },
      {
      ...
      },
      {
        ...
        component: () => import("../views/ImsQuestionnaireReview.vue"),
        props: (route) => ({
          questionnaireCode: route.params.questionnaire_code,
        }),
      },
      {
        ...
        component: () =>
          import("../views/Set/setSettlementVisitorOverview.vue"),
      },
      {
...
router.beforeEach((to: any, from: any, next: Function) => {
  ...
  const loginJrsUser: Function = (username: string) => {
    ...
    store
      .dispatch("auth/loginAzure", {
        userName: username,
        MsalObject: myMSALObj,
      })
      .then((res: any) => {
        setupMenu(to) // Setup the user menus and continue to route
          .then((flatMenus: any) => {
            // Get list of user available manus.
            let destinationMenuIndex = flatMenus.findIndex(
              (menuItem: any) => menuItem.path == to.path
            );
            let destinationMenu = flatMenus.find(
              (menuItem: any) => menuItem.path == to.path
            );
            // If the destination route is not in the list of user menus --> redirect to 404
            if (
              to.path != `/${i18n.locale}/404` &&
...
async function setupMenu(to: any) {
  let language =
    to.params.lang != undefined ? to.params.lang : i18n.fallbackLocale; // Get current language (From url or default)

  let user: User = store.getters["auth/loggedUser"]; // Get User
  let userUid: string | null =
    user != null && user.userUid ? user.userUid : null; // Get User UID

  if (userUid != null) {
    return store
      .dispatch("setUserMenu", userUid) // Set User menu
      .then((res) => {
        return store.dispatch("setUserLocale", language); // Set user local
      })
      .then((res) => {
        return store.dispatch("caclculateFlatMenu");
      }); // Calculate flat menu
  } else {
...
async function setupCurrentModule(currentMenu: any) {
  ...
  if (currentMenu.moduleId) {
    let current_module = moduleLists.find(
      (ele: any) => ele.moduleId == currentMenu.moduleId
    );
